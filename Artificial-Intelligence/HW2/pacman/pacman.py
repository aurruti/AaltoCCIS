...
### You can come up with your own names for the different moves
        def forward(x,y,d):
            if d == "N":
                return ("Forward", PacManState(x,y+1, "N",self.grid))
            elif d == "S":
                return ("Forward", PacManState(x,y-1, "S",self.grid))
            elif d == "E":
                return ("Forward", PacManState(x+1,y, "E",self.grid))
            elif d == "W":
                return ("Forward", PacManState(x-1,y, "W",self.grid))
        def left(x,y,d):
            if d == "N":
                return ("Turn Left", PacManState(x-1,y,"W",self.grid))
            elif d == "S":
                return ("Turn Left", PacManState(x+1,y,"E",self.grid))
            elif d == "E":
                return ("Turn Left", PacManState(x,y+1,"N",self.grid))
            elif d == "W":
                return ("Turn Left", PacManState(x,y-1,"S",self.grid))
        def right(x,y,d):
            if d == "N":
                return ("Turn Right", PacManState(x+1,y,"E",self.grid))
            elif d == "S":
                return ("Turn Right", PacManState(x-1,y,"W",self.grid))
            elif d == "E":
                return ("Turn Right", PacManState(x,y-1,"S",self.grid))
            elif d == "W":
                return ("Turn Right", PacManState(x,y+1,"N",self.grid))
        def turn_around(x,y,d):
            if d == "N":
                return ("Turn Around", PacManState(x,y-1,"S",self.grid))
            elif d == "S":
                return ("Turn Around", PacManState(x,y+1,"N",self.grid))
            elif d == "E":
                return ("Turn Around",PacManState(x-1,y,"W",self.grid))
            elif d == "W":
                return ("Turn Around", PacManState(x+1,y,"E",self.grid))
        successors = []
        if self.d == "N":
            if not self.grid.occupied(self.x,self.y+1):
                successors.append(forward(self.x,self.y,self.d))
            if not self.grid.occupied(self.x-1,self.y):
                successors.append(left(self.x,self.y,self.d))
            if not self.grid.occupied(self.x+1,self.y):
                successors.append(right(self.x,self.y,self.d))
            if successors == []:
                successors.append(turn_around(self.x,self.y,self.d))
        elif self.d == "S":
            if not self.grid.occupied(self.x,self.y-1):
                successors.append(forward(self.x,self.y,self.d))
            if not self.grid.occupied(self.x+1,self.y):
