...
# Create an h-function for a goal state in MAPPState
...
if __name__ == "__main__":
    ...
    grid_G = MAPPState([(0,0),(4,4),(0,4),(4,0)],nrows=5,ncols=5,walls=[])
    ...
    print("to state")
    print(grid_G)
    print(f"is h_max = {h_max(grid_S)}, h_sum = {h_sum(grid_S)}.")

    grid_T = MAPPState([(0,0),(3,3),(0,1),(1,0)],nrows=5,ncols=5,walls=[])
    print("Distance heuristics from state")
    print(grid_T)
    print("to state")
    print(grid_G)
    print(f"is h_max = {h_max(grid_T)}, h_sum = {h_sum(grid_T)}.")
