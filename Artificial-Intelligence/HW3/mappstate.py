...
class MAPPState(state.State):
    ...
    def apply(self,action):
        ...
        return MAPPState(action.target, self.nrows, self.ncols,
                             walls = self.walls)
        ...
        for locations in itertools.product(*moves):
            ...
            if all(not (locations[i] == locations[j] \
                ...
                succ.append((Action(self.agents,locations, cost),
                            MAPPState(locations, nrows = self.nrows,
    ...
    def create_from_string(mapp_strings):
        ...
        return MAPPState([agents[a] for a in sorted(agents)],
                             nrows, ncols,
                             walls = walls)
...
if __name__ == "__main__":
    ...
    x = MAPPState([(0,0), (5,4)], nrows=7, ncols=5)
    # Creating a state with walls.
    y = MAPPState([(1,0), (9,9)], walls = {(5,5),(1,1),(7,9)})
    # Create the same state, but parse it.
    z = MAPPState.create_from_string(
        ["..........",
         "0#........",
         "..........",
         "..........",
    ...
    print("z=")
    print(str(z))
    print(f"Moreover, y and z are the same state: {y == z}.")
