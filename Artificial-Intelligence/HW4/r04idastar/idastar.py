...
def doDFS(s,g,bound,goaltest,h,path):
    ...
    f = g + h(s)
    if f > bound:
        return f  # Exceeding current bound, return the next bound
    if goaltest(s):
        return []  # Already the goal
    minbound = float('inf') 
    for action, successor in s.successors(): 
        if successor not in path:  # Check for cycles
            path.append(successor)
            res = doDFS(successor, g + action.cost, bound, goaltest, h, path)
            if res is None:
                return None  # Exceeded time
            if isinstance(res, list):
                return [action] + res  # Found the solution, return the action list
            if res < minbound:
                minbound = res  # Update minimum bound
            path.pop()  # Backtrack
    return minbound  # Return the minimum bound encountered during DFS
