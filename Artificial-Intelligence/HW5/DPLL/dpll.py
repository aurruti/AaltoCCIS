...
def unit_propagation(valuation, clauses):
    ...
    * Otherwise, if all literals of a clause are False, then the whole clause
    ...
    while count < len(clauses):
        clause = clauses[count]
        clause_is_true = False
        unasigned_var = []
        for literal in clause:
            variable, polarity = literal
            if variable in valuation:
                if valuation[variable] == polarity:
                    clause_is_true = True
                    break
            else:
                unasigned_var.append(variable)
                unasigned_var.append(polarity)
        if not(clause_is_true) and len(unasigned_var) == 2:
            valuation[unasigned_var[0]] = unasigned_var[1]
            count = -1
        elif not(clause_is_true) and len(unasigned_var) == 0:
            return (False, {})
        count += 1
...
def dpll(valuation, clauses, variables):
    ...
    stack = [(valuation, clauses)] 

    while stack:
        valuation, clauses = stack.pop()
        ...
        all_assigned = all(var in valuation for var in variables)
        if all_assigned:
            return True, valuation

        # Choose new variable for case analysis
        branching_variable = next(var for var in variables if var not in valuation)

        # Try Variable is True
        true_valuation = valuation.copy()
        true_valuation[branching_variable] = True
        stack.append((true_valuation, clauses))

        # Try Variable is False
        false_valuation = valuation.copy()
        false_valuation[branching_variable] = False
        stack.append((false_valuation, clauses))
